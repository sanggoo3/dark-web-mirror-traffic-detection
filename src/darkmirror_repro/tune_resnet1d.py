...
from .models import DirectionResNet1D
...
def candidate_grid() -> list[dict]:
    rows = []
    idx = 1
    for channels, dropout, lr, batch_size in itertools.product(
        [(32, 64, 128), (64, 128, 128)],
        [0.0, 0.1],
        [0.001, 0.0005],
        [64, 128],
    ):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the ResNet1D hyperparameter grid on the final seq_len=506 input.")
    parser.add_argument(
        "--data",
    ...
    parser.add_argument("--out-dir", default="results/reproduced_resnet1d_tuning")
    ...
    df.to_csv(out_dir / "resnet1d_hyperparameter_tuning_results.csv", index=False, encoding="utf-8-sig")
    paper_cols = [
        "rank",
        "candidate_id",
        "channels",
    ...
    paper.to_csv(out_dir / "resnet1d_hyperparameter_tuning_results_paper.csv", index=False, encoding="utf-8-sig")
    (out_dir / "resnet1d_hyperparameter_tuning_results_paper.md").write_text(
        "\n".join(["# ResNet1D Hyperparameter Tuning Results", "", paper.to_markdown(index=False, disable_numparse=True), ""]),
        encoding="utf-8",
    )
    print(f"[done] ResNet1D tuning results: {out_dir}")
